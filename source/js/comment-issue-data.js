...
var repoIssuesUrl = "https://api.github.com/repos/finlu/blog_comment/issues";
// 对应仓库 clientId、clientSecret 关于这两个参数的安全问题，查看 https://removeif.github.io/2019/09/19/博客源码分享.html#1-热门推荐，最新评论：
var clientId = "c150f6b083aad0ff7258";
var clientSecret = "3205b241fb368308b28de39648469793b7e95a53";
var authorizationToken = "Basic " + btoa(clientId + ":" + clientSecret);
// 写comment count值
var reqCommentCountUrl =
  repoIssuesUrl + "?t=" + new Date().getTime() + "&labels=Gitalk,";
// 评论缓存key
var COMMENT_CACHE_KEY = "commentKey";

// 管理员名称,评论时添加 [博主] 后缀
var ADMIN_NAME = "finlu";
...
function loadCommentDataAndRender() {
  ...
  ajaxReqForGitHub(
    repoIssuesUrl + "/comments?sort=created&direction=desc&per_page=7&page=1",
    function(data) {
      fillComments(data);
    }
  );
  ...
  if (COMMENT_ARR.length > 0) {
    ...
    for (var i = 0; i < COMMENT_ARR.length; i++) {
      ...
      htmlContentWidget +=
        ...
        item.userAvatar +
        "'/></a>" +
        "<div class='item-header-text'><a href='" +
        item.userUrl +
        "' target='_blank'>" +
        item.userName +
        "</a>&nbsp;发表于" +
        getDateDiff(new Date(item.date).getTime()) +
        "</div>" +
...
function loadIndexHotData() {
  ...
  if ($("#index_hot_div").length > 0) {
    ...
    ajaxReqForGitHub(repoIssuesUrl + "?per_page=10&sort=comments", function(
      result
    ) {
      $.each(result, function(i, item) {
        // 标签配色
        ...
        hotContent +=
          '<a href ="' +
          item.body.substr(0, item.body.indexOf("\n") - 1) +
          '"target="_blank"' +
          classDiv +
          ">" +
...
function loadIssueData() {
  setTimeout(function() {
    ...
    if (
      COMMENT_CACHE == "" ||
      COMMENT_CACHE == null ||
      new Date().getTime() - COMMENT["date"] > 60 * 1000
    ) {
    ...
    console.clear();
    console.log(
      "~~~~欢迎光临！记得有时间多来看看哦，https://removeif.github.io/ ~~~~"
    );
  }, 500);
}

function loadSelfGitalk() {
  //加载碎碎念
